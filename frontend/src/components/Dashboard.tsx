...
const usageConfig = [
  { label: 'Session', icon: Activity, accent: 'from-amber-500/20 to-amber-600/5', iconColor: 'text-amber-400', borderColor: 'border-amber-500/20' },
  { label: 'Today', icon: Clock, accent: 'from-orange-500/20 to-orange-600/5', iconColor: 'text-orange-400', borderColor: 'border-orange-500/20' },
  ...
  return (
    <div className="space-y-5 max-w-[1400px]">
      ...
      {!stats.running && recentHistory.length > 0 && (
        <div className="animate-fade-in-up stagger-2 bg-forge-surface rounded-xl border border-forge-border p-4 card-hover">
          ...
          <ResponsiveContainer width="100%" height={64}>
            <AreaChart data={recentHistory}>
              ...
              <Area type="monotone" dataKey="downloadBps" stroke="#f59e0b" fill="url(#sparkGradient)" strokeWidth={2} dot={false} isAnimationActive={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {usageConfig.map((cfg, i) => {
            ...
            return (
              <div
                key={cfg.label}
                className={`animate-fade-in-up stagger-${i + 3} group relative bg-forge-surface rounded-xl border ${cfg.borderColor} overflow-hidden card-hover`}
              >
                ...
                <div className="relative p-4">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <ArrowDownCircle size={13} className="text-orange-400 flex-shrink-0" />
                      <span className="text-sm font-bold font-mono text-zinc-200 tabular-nums">{formatBytes(data.dl)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <ArrowUpCircle size={13} className="text-slate-400 flex-shrink-0" />
      ...
      {/* ================================================================ */}
      {/* NEW DATA PANELS - 2-column grid                                 */}
      {/* ================================================================ */}

      {/* Row 1: Peak Performance + Throttle Monitor */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {/* Peak Performance */}
        <div className="animate-fade-in-up bg-forge-surface rounded-xl border border-forge-border overflow-hidden card-hover">
          <div className="p-4">
        ...
        {/* Throttle Monitor */}
        <div className={`animate-fade-in-up bg-forge-surface rounded-xl border overflow-hidden card-hover ${
          activeThrottles.length > 0 ? 'border-red-500/30' : 'border-forge-border'
        }`}>
          <div className="p-4">
        ...
        <div className="animate-fade-in-up bg-forge-surface rounded-xl border border-forge-border p-4 card-hover">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-3 text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-orange-500" /> Download</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-slate-500" /> Upload</span>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={dailyChartData} barGap={2}>
              ...
              />
              <Bar dataKey="dl" fill="#f97316" radius={[4, 4, 0, 0]} />
              <Bar dataKey="ul" fill="#64748b" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
            ...
            {speedChartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={100}>
                <LineChart data={speedChartData}>
                  ...
                  />
                  <Line type="monotone" dataKey="download" stroke="#f59e0b" strokeWidth={2} dot={{ r: 2, fill: '#f59e0b' }} />
                  <Line type="monotone" dataKey="upload" stroke="#94a3b8" strokeWidth={2} dot={{ r: 2, fill: '#94a3b8' }} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
              ...
              <div className="space-y-2 max-h-[130px] overflow-y-auto">
                {schedules.slice(0, 5).map(s => (
                  <div key={s.id} className={`flex items-center justify-between py-2 px-3 rounded-lg ${
                    s.enabled ? 'bg-teal-500/5 border border-teal-500/10' : 'bg-forge-raised/30'
                  }`}>
                    <div className="flex items-center gap-2.5">
                      <div className={`w-1.5 h-1.5 rounded-full ${s.enabled ? 'bg-teal-400' : 'bg-zinc-600'}`} />
                      <div className="flex gap-0.5">
                        {[0,1,2,3,4,5,6].map(d => (
                          <span key={d} className={`text-[9px] font-mono w-4 text-center ${
                            s.daysOfWeek.includes(d)
                              ? s.enabled ? 'text-teal-400 font-bold' : 'text-zinc-400 font-bold'
                              : 'text-zinc-700'
                          }`}>{dayLabels[d][0]}</span>
                        ))}
                      </div>
      ...
      {/* Row 5: System Status + Network Config */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {/* System Status */}
        {updateStatus && (
          <div className="animate-fade-in-up bg-forge-surface rounded-xl border border-forge-border overflow-hidden card-hover">
